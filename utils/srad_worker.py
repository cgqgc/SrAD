...
class SrADWorker:
    def __init__(self, opt):
        super(SrADWorker, self).__init__()
        ...
        self.net = UNet(
            in_channels=self.opt.model["in_c"], n_classes=self.opt.model["in_c"]
        ).cuda()
    ...
    def set_dataloader(self, test=False):
        ...
        if self.opt.dataset == "brats":
            if not test:
                self.train_set = BraTSAD(
                    main_path=data_path,
                    img_size=self.opt.model["input_size"],
                    transform=train_transform,
                    mode="train",
                )
            self.test_set = BraTSAD(
                main_path=data_path,
                img_size=self.opt.model["input_size"],
            ...
            if not test:
                self.train_set = PseADSeg(
                    main_path=data_path,
                    img_size=self.opt.model["input_size"],
                    transform=train_transform,
                    mode="train",
                )
            self.test_set = PseADSeg(
                main_path=data_path,
                img_size=self.opt.model["input_size"],
                transform=test_transform,
                mode="test",
        ...
        self.test_loader = DataLoader(
            self.test_set, batch_size=1, shuffle=False, pin_memory=True, num_workers=0
        )
    ...
    def set_test_loader(self):
        ...
        print(
            "=> Set test dataset: {} | Input size: {} | Batch size: {}".format(
                self.opt.dataset, self.opt.model["input_size"], 1
            )
        )
    ...
    def save_checkpoint(self, file_name=None):
        if file_name is not None:
            torch.save(
                self.net.state_dict(),
                os.path.join(self.opt.train["save_dir"], "checkpoints", file_name),
            )
        else:
            torch.save(
                self.net.state_dict(),
                os.path.join(self.opt.train["save_dir"], "checkpoints", "model.pt"),
            )

    def load_checkpoint(self, file_name=None):
        if file_name is not None:
            model_path = os.path.join(
                self.opt.train["save_dir"], "checkpoints", "model.pt"
            )
            self.net.load_state_dict(
                torch.load(
                    model_path,
                    map_location=torch.device("cuda:{}".format(self.opt.gpu)),
                )
            )
            print("=> Load model from {}".format(model_path))
        else:
            model_path = os.path.join(
                self.opt.train["save_dir"], "checkpoints", "model_last.pt"
            )
            self.net.load_state_dict(
                torch.load(
                    model_path,
    ...
    def run_eval(self):
        ...
        with open(
            f"{self.opt.test['save_dir']}/{self.opt.model['name']}_{self.opt.dataset}_score.csv",
            "w",
        ) as f:
    ...
    def add_noise(self, x):
        ...
        ns = torch.normal(
            mean=torch.zeros(x.shape[0], x.shape[1], self.noise_res, self.noise_res),
            std=self.noise_std,
        ).cuda()

        ns = F.interpolate(
            ns, size=self.opt.model["input_size"], mode="bilinear", align_corners=True
        )
    ...
    def evaluate(self):
        ...
        for idx_batch, data_batch in tqdm(enumerate(self.test_loader)):
            ...
            anomaly_score_map = (
                self.criterion(img, net_out, anomaly_score=True, keepdim=True)
                .detach()
                .cpu()
            )
            # 原
        ...
        if self.pixel_metric:
            ...
            pix_ap = metrics.average_precision_score(
                test_masks.numpy().reshape(-1), test_score_maps.numpy().reshape(-1)
            )
            pix_auc = metrics.roc_auc_score(
                test_masks.numpy().reshape(-1), test_score_maps.numpy().reshape(-1)
            )

            best_dice, best_thresh = compute_best_dice(
                test_score_maps.numpy(), test_masks.numpy()
            )
    ...
    def run_train(self):
        ...
        for epoch in range(1, num_epochs + 1):
            ...
            if epoch == 1 or epoch % self.opt.train["eval_freq"] == 0:
                eval_results, epoch_auc, test_labels, test_scores, test_names = (
                    self.evaluate()
                )

                t = time.time() - t0
                print(
                    "Epoch[{:3d}/{:3d}]  Time:{:.1f}s  loss:{:.5f}".format(
                        epoch, num_epochs, t, train_loss
                    ),
                    end="  |  ",
                )
                ...
                if epoch_auc > max_auc:
                    ...
                    plt.legend()
                    plt.savefig(
                        os.path.join(self.opt.test["save_dir"], "confusion matrix.png")
                    )
                    plt.close()

                    tn, fp, fn, tp = C.ravel()
                    f1_score = metrics.f1_score(
                        test_labels, test_scores, average="macro"
                    )
                    specificity = tn / (tn + fp)
                    recall_score = tp / (tp + fn)  # sensitivity
                    precision_score = tp / (tp + fp)
                    ...
                    df.to_csv(
                        os.path.join(self.opt.test["save_dir"], "cls_metrics.csv"),
                        index=False,
                    )
